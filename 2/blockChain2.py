...
def make_hash(blockID: str, prev_hash: bytes, data: str) -> bytes:
    s = blockID.encode() + prev_hash + data.encode() # 1048616
    ...
    '''
    print((s2.value[12:32]).hex())
    '''
    
    return bytes.fromhex((s2.value[12:32]).hex()) # 25~64 총 40개만 가져옴


def add_block(i):
    global block75Data
    prev_hash = blockChain[-1][1]
    blockID = format((i//16)*10 + i%16,'040') # 20byte 160bit
    data = randData()
    if i==75:
        block75Data = data
...
block75Data = block75Data[:99] + '.' + block75Data[100:] # (여기를 주석 해제), data의 100번째 index를 .으로 교체하여 검증
